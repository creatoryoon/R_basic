...
# 구동 방식별로 도시 연비가 다른지 알아보려 한다. 분석을 하기 전에 우선 두 변수에 이상치가 있나 확인하자.
...
#Q1 drv에 이상치가 있는지 확인해라. 이상치를 결측 처리 후 이상치가 사라졌는지 확인하자. 결측 처리를 할 때는 %in%  기호를 활용해라.
# drv가 정상적인 값이면 원래 값을 유지하고 그렇지 않으면 NA를 부여하는 코드를 작성하면 된다. 정상적인 값이 열ㅓ개 있으니 %in%와 c()를 조합해 코드를 간결하게 만들어 보자
table(mpg$drv)
...
#Q2 상자 그림을 이용해 cty에 이상치가 있는지 확인해라. 상자 그림의 통계치를 이용해 정상 범위를 벗어난 값을 결측 처리한 수 다시 상자 그림을 만들어 이상치가 사라졌는지 확인해라.
